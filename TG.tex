\documentclass[12pt,letterpaper,oneside]{report}

%---------------------PACKAGES
\usepackage[margin=3cm]{geometry}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage[spanish]{babel}
\usepackage[fixlanguage]{babelbib}
\selectbiblanguage{spanish}
\usepackage{url}
\usepackage{float}
%-----------------------------

\title{Trust Me}
\author{{Brayan Arango} \\ {Wilder Alcala}}

%-----------------CONFIG
\renewcommand{\labelitemi}{$\bullet$}
\renewcommand{\labelitemii}{$-$}
\setcounter{tocdepth}{4}
\setcounter{secnumdepth}{4}
%-----------------------

\begin{document}
	
	%-----------------INDEX
	\include{start/coverPage}
	\include{start/backCover}

	\pagenumbering{Roman}
	\urlstyle{same}
	\include{start/acceptanceNote}
	\include{start/dedication}
	
	%--------------------TABLES
	\tableofcontents
	\listoffigures
	%\listoftables
	
	%-----------------CONTENT
	\include{start/intro}
	
	\part{CONTEXTUALIZACIÓN DE LA INVESTIGACIÓN}
	
		\chapter{DESCRIPCION DE LA INVESTIGACIÓN}
			\input{description/problem}
			\input{description/objectives}
			\input{description/justification}
			\input{description/scopes}
			\input{description/hypothesis}
			\input{description/framework}
			\input{description/methodology}
			
	\part{DESARROLLO DE LA INVESTIGACIÓN}
		
		\chapter{METODOLOGÍA APLICADA}
			\input{development/scrum}
			\input{development/cases}
			\input{development/wireframe}
			\input{development/database}
		
		\chapter{ORGANIZACIÓN}
			\input{development/intro}
			\input{development/trustme}
			\input{development/mission}
			\input{development/view}
			\input{development/objectives}
			\input{development/beginning}
			\input{development/services}
			\input{development/products}
			%\input{development/processes}
			%\input{development/functions}
			
		\chapter{ARQUITECTURA EMPRESARIAL}
			\input{development/deal}
			%\input{development/application}
			%\input{development/technology}

	\part{CIERRE DE LA INVESTIGACIÓN}

		\chapter*{RESULTADOS Y DISCUSIÓN}
		\addcontentsline{toc}{chapter}{RESULTADOS Y DISCUSIÓN}
			\input{closing/results}

		\chapter*{CONCLUSIONES}
		\addcontentsline{toc}{chapter}{CONCLUSIONES}
			\input{closing/conclusions}
			\input{closing/originals}
			\input{closing/works}
			
		\chapter*{PROSPECTIVA DEL TRABAJO DE GRADO}
		\addcontentsline{toc}{chapter}{PROSPECTIVA DEL TRABAJO DE GRADO}
			\input{closing/future}
			
	\renewcommand{\bibname}{REFERENCIAS}	
	\bibliographystyle{ieeetr}
	\addcontentsline{toc}{part}{REFERENCIAS}
	\bibliography{bibliography/articles,bibliography/books,bibliography/online}
	
	\chapter*{ANEXOS}
	\addcontentsline{toc}{part}{ANEXOS}
		\input{annexes/annexA}
	

\end{document}          
